...
export async function generateMetadata({ params, searchParams }: PageProps): Promise<Metadata> {
    ...
    const filters = parseSearchParams(raw, { fuel: [fuel] });
    ...
    return generateListingMetadata({
        ...
        canonicalPath: `/used-${fuel}-cars`,
        page: raw.page ? Number(raw.page) : undefined,
    }, hasComplexFilters);
}

export default async function FuelTypeCarsPage({ params, searchParams }: PageProps) {
    const { fuel } = await params;
    const raw = await searchParams;
    const fuelName = fuel.toUpperCase() === 'CNG' ? 'CNG' : fuel.charAt(0).toUpperCase() + fuel.slice(1);

    const seoInputs = { fuel, total: 0 };
    ...
    return (
        <SharedListingLayout
            searchParams={raw}
            filterOverrides={{ fuel: [fuel] }}
            title={`Used ${fuelName} Cars`}
